...
delta_mat_A = np.matrix(np.zeros((k,k)))
...
def remplir_delta_A(l):
    ...
    for d in D:
        ...
        for i in range(len_currentA):
            if i == len_currentA -1:
                delta_mat_A[somme_d + i, somme_d + i -1] = l*0.01
                # delta_mat_A[somme_d + i, somme_d + i] = l*0.01
        somme_d += d+1
        index_d += 1
...
"""def remplir_U_infty():
    ...
    R = np.identity(N)
    for t in range(1, t_max):
        U = np.linalg.inv(mat_B)*(np.identity(mat_A.shape)-mat_A)"""
...
l = 0
L_pole = [np.zeros(500, "complex") for i in range(k)] 
ZZ=[]
compteur = 0
delta_max= 0.75
for dl in np.arange(0, delta_max, 0.001):
    l += dl
    remplir_delta_A(l)
    Z = mat_A + delta_mat_A - mat_B*K
    z = np.linalg.eigvals(Z)
    ZZ.append(z.tolist())
...
for i, row in enumerate(M.T):
    color = cmap(i / (len(M.T) - 1))  # Normalize i to [0, 1]
    ax.scatter(row.real[0], row.imag[0], marker='X', color=color, label=f'Row {i + 1}')
    ax.scatter(row.real[-1], row.imag[-1], marker='o', color=color)
    ax.plot(row.real, row.imag, color=color)

plt.title("Evans diagram")
plt.text(0, 0.7, "a1 = 0.95, a2 = 0.9 ")
plt.text(0, 0.5, "0 < b1 < 0.95, 0 < b2 < 0.9")
